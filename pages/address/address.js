...
const app = getApp();
const { getAddressList, delAddress } = require('../../utils/api.js');
...
Page({
  ...
  getAddressList() {
    ...
    getAddressList().then(res => {
      console.log(res)
      let addressList = res.data.code.list;
      that.setData({
        addressList
      })
    })
  ...
  delAddress() {
    ...
    this.closeDiaLog();
    delAddress({ address_id: this.data.currentAddressId }).then(res => {
      wx.hideLoading()
  ...
  showDiaLog(ev) {
    ...
    let address_id = ev.currentTarget.dataset.id;
    this.setData({
      showDiaLog: true,
  ...
  changeDefault(ev) {
    ...
    changeDefaultAddress(app.globalData.openid, id).then(res => {
      console.log(res);
