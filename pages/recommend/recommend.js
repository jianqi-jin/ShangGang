...
Page({
  ...
  getData: function(idx) {
    ...
    api.getReferList({
      ...
      let key = `list[${idx}]`;
      newData.page++;
      newData.data = newData.data.concat(res.data.code.arr);
      this.setData({
        banner: res.data.code.image
      })
  ...
  onShareAppMessage: function(e) {
    ...
    if (e.target) {
      ...
      util.request(api.serverUri + `tj/fx&openid=${app.globalData.openid}`, 'POST', {
        id: id,
        type: 1
      }).then((res) => {
        // 统计
