...
function searchBank (card){
  return new Promise(function (resolve, reject) {
    wx.request({
      url: `https://ccdcapi.alipay.com/validateAndCacheCardInfo.json?cardNo=${card}&cardBinCheck=true`, // 仅为示例，并非真实的接口地址
      header: {
        'content-type': 'application/json' // 默认值
      },
      success(res) {
        if (BANK[res.data.bank]){
          let json = { 
            bank: res.data.bank,
            name: BANK[res.data.bank],
            img: `https://apimg.alipay.com/combo.png?d=cashier&t=${res.data.bank}`
          }
          resolve(json);
        }else{
          reject("银行卡号有误");
